...
| `operatorName` | String | Y | 통신망 기준 구분값 (`KT 알뜰폰`, `SK 알뜰폰`, `LG 알뜰폰`) |
| `providerName` | String | Y | 실제 사용 통신업체명 (예: "프리티", "헬로모바일") |
...
| `reminderDaysBeforeEnd` | Int | N | 미리 알림 일수(일) (기본값: 15) |
...
### 6.1 요금제 등록
- **입력**: 통신사명, 사용 통신업체명, 요금제명, 가입일, 프로모션 기간, 최소유지, 위약금, 월 요금(선택), 미리 알림 일수
- **처리**: 
  1. 모든 필수 필드 검증
...
### 7.1 필수 필드
- `operatorName`: 드롭다운 선택값 (`KT 알뜰폰`, `SK 알뜰폰`, `LG 알뜰폰`)
- `providerName`: 길이 1~30자
- `activationDate`: 과거 날짜 (오늘 이후 선택 불가)
- `promotionMonths`: 1 이상 정수

### 7.2 선택 필드 (기본값)
- `activationDate`: 기본값 오늘 날짜
- `promotionMonths`: 기본 0
